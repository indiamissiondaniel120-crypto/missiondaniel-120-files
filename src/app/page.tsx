...
function Dashboard() {
  ...
  const activityQuery = useMemo(() => {
    ...
    return query(
      collection(db, 'students', user.id, 'activity'),
  ...
  const logContentActivity = (
    material: Material,
    type: 'pdf_view' | 'video_view',
    duration: number
  ) => {
  ...
  const handleCloseMaterial = () => {
    if (activeMaterial && viewStartTime.current) {
      ...
      logContentActivity(
        activeMaterial,
        activeMaterial.type === 'pdf' ? 'pdf_view' : 'video_view',
        duration
      );
          ...
          <div className="max-w-6xl mx-auto space-y-8">
            {showAdminPanel ? (
              ...
              <>
                <section>
                  <div
                    className={`${isAdmin ? 'bg-accent' : isMentor ? 'bg-orange-500' : 'bg-primary'} rounded-3xl p-10 text-white relative overflow-hidden shadow-xl`}
                  >
                      ...
                      {user?.role === 'student' && (
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
                          ...
                          <div className="bg-white/10 p-4 rounded-2xl border border-white/10 backdrop-blur-md">
                            ...
                            <p className="text-sm font-bold truncate">{user.id}</p>
                          </div>
                        </div>
                      )}
                        ...
                        {visibleCourses.map((course) => (
                          ...
                          >
                            ...
                            <CardContent className="p-5">
                              ...
                              <Button
                                variant="link"
                                className="p-0 text-accent font-bold mt-4"
                              >
                          ...
                          >
                            <CardContent className="flex items-center justify-between p-4">
                              ...
                              <Button
                                size="sm"
                                variant={
                                  activeMaterial?.id === note.id ? 'outline' : 'ghost'
                                }
                                onClick={() =>
                                  activeMaterial?.id === note.id
                                    ? handleCloseMaterial()
                                    : handleOpenMaterial(note)
                                }
                              >
                                {activeMaterial?.id === note.id
                                  ? 'Stop Reading'
      ...
      {user?.role === 'student' && user.mentorId && (
        <div className="fixed bottom-6 right-6 z-50">
          <Popover>
            ...
            <PopoverContent
              className="w-[350px] p-0 mr-6 mb-2 border-none shadow-2xl rounded-2xl overflow-hidden"
              align="end"
            >
              <ChatInterface
...
function UnreadDot({ userId, mentorId }: { userId: string; mentorId: string }) {
  ...
  return (
    <span className="absolute top-0 right-0 h-4 w-4 bg-red-500 border-2 border-white rounded-full" />
  );
}
