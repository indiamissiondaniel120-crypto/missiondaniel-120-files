...
import { UserPlus, Activity, FileText, PlayCircle, Download, UserRound, GraduationCap, Edit2, BookOpen, Trash2, Plus, Loader2, Library, ListChecks, Search, RefreshCw, Layers, Link, MessageSquare, Video, Clock, Globe, FileSpreadsheet, RefreshCcw, Sparkles } from 'lucide-react'
...
export function ActivityViewer({ student, mentors }: { student: any, mentors: any[] }) {
  ...
  const downloadAttendanceCSV = () => {
    ...
    const headers = ["Date", "Time", "Event Type", "Item/Title", "Chapter", "Duration (Seconds)"]
    const rows = activities.map((log: any) => {
      ...
      return [date, time, type, item, chapter, duration]
    })
    ...
    link.setAttribute("download", `student_report_${student.name.replace(/\s+/g, '_')}_${student.id}.csv`);
      ...
      <DialogContent className="max-w-2xl h-[80vh] flex flex-col p-0 w-[95vw] rounded-2xl">
        <DialogHeader className="p-6 md:p-8 border-b flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="min-w-0 text-left">
            <DialogTitle className="truncate text-base md:text-xl font-black">Activity: {student.name}</DialogTitle>
            <DialogDescription className="text-xs font-bold">Comprehensive engagement & usage logs</DialogDescription>
          </div>
          <Button variant="outline" size="sm" onClick={downloadAttendanceCSV} className="gap-1.5 h-10 w-full sm:w-auto rounded-xl border-accent/20 text-xs font-black">
            <FileSpreadsheet size={14} /> Full CSV Report
          </Button>
        </DialogHeader>
        <ScrollArea className="flex-1 p-6 md:p-8">
          {loading ? <div className="flex flex-col items-center py-20 gap-4"><Loader2 className="animate-spin h-8 w-8 text-primary" /><p className="font-black text-xs">Loading analytics...</p></div> : (
            <div className="space-y-3">
              {activities?.length === 0 && <p className="text-center text-muted-foreground py-8 italic text-xs font-bold">No activity recorded for this student yet.</p>}
              {activities?.map((log: any, i: number) => (
                <div key={i} className="p-3 md:p-4 border rounded-xl flex items-center justify-between bg-muted/10 hover:bg-muted/20 transition-colors">
                  <div className="flex flex-col gap-0.5 min-w-0">
                    <span className="font-black text-[10px] md:text-sm uppercase text-primary truncate max-w-[150px] md:max-w-none tracking-tight">{log.type?.replace(/_/g, ' ')}</span>
                    <span className="text-[9px] md:text-xs text-muted-foreground truncate font-bold">
                      {log.metadata?.title ? `${log.metadata.title} (Ch ${log.metadata.chapter})` : ''}
                    </span>
                  </div>
                  <div className="text-right shrink-0">
                    <div className="text-[9px] md:text-xs font-bold text-foreground">{log.timestamp?.toDate()?.toLocaleTimeString()}</div>
                    <div className="text-[8px] md:text-[10px] text-muted-foreground font-bold">{log.timestamp?.toDate()?.toLocaleDateString()}</div>
                    {log.duration > 0 && <Badge variant="secondary" className="text-[8px] md:text-[10px] text-accent font-black mt-1 h-5 px-1.5">{log.duration}s</Badge>}
                  </div>
                </div>
              ))}
            </div>
        ...
        <DialogFooter className="p-4 md:p-6 border-t bg-muted/20">
          <DialogTrigger asChild><Button className="w-full h-12 md:h-14 rounded-xl font-black">Close Analytics</Button></DialogTrigger>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
...
export function StudentManagement() {
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500">
      ...
      <Dialog open={!!selectedChapterInfo} onOpenChange={() => setSelectedChapterInfo(null)}><DialogContent className="max-w-md w-[95vw] rounded-xl"><DialogHeader><DialogTitle className="text-base font-black">{selectedChapterInfo?.className} - {selectedChapterInfo?.subjectName}</DialogTitle><DialogDescription className="text-xs font-bold">Ch {selectedChapterInfo?.chapter} Resources</DialogDescription></DialogHeader><div className="space-y-3 py-4 max-h-[60vh] overflow-y-auto"><div className="space-y-3">{selectedChapterInfo?.materials.map((m: any) => (<div key={m.id} className="p-3 rounded-xl border bg-muted/20 flex flex-col gap-2"><div className="flex items-center justify-between"><div className="flex items-center gap-2 min-w-0">{m.type === 'video' ? <PlayCircle className="text-red-500 h-4 w-4" /> : <FileText className="text-blue-500 h-4 w-4" />}<span className="font-bold text-xs truncate">{m.title}</span></div><div className="flex items-center gap-1"><Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => setEditingMaterial(m)}><Edit2 size={12} /></Button><Badge variant="outline" className="text-[8px]">{m.type}</Badge></div></div><a href={m.url} target="_blank" rel="noopener noreferrer" className="text-[10px] text-primary truncate hover:underline flex items-center gap-1"><Link size={10} /> {m.url}</a></div>))}</div></div><DialogFooter><Button variant="ghost" onClick={() => setSelectedChapterInfo(null)} className="w-full h-12">Close</Button></DialogFooter></DialogContent></Dialog>
...
function LiveAttendanceViewer({ session, onClose }: { session: any, onClose: () => void }) {
  ...
  const downloadReport = () => {
    ...
    const link = document.createElement("a"); link.setAttribute("href", encodeURI(csvContent)); link.setAttribute("download", `live_attendance_${session.title.replace(/\s+/g, '_')}.csv`); document.body.appendChild(link); link.click();
  };
  if (!session) return null;
  return (<Dialog open={!!session} onOpenChange={onClose}><DialogContent className="max-w-3xl h-[85vh] flex flex-col p-0 overflow-hidden w-[95vw] rounded-2xl"><DialogHeader className="p-6 md:p-8 border-b"><div className="flex justify-between items-center gap-4"><div><DialogTitle className="text-lg md:text-2xl font-black">{session.title}</DialogTitle><DialogDescription className="font-bold text-xs">{session.className} • {session.subjectName}</DialogDescription></div><Button onClick={downloadReport} className="bg-accent h-10 md:h-12 gap-2 text-xs font-black" disabled={logs.length === 0}><Download size={14} /> Export CSV</Button></div></DialogHeader><ScrollArea className="flex-1 p-4 md:p-8">{loading ? <div className="flex flex-col items-center py-20 gap-4"><Loader2 className="animate-spin h-8 w-8 text-primary" /><p className="font-black text-xs text-muted-foreground">Syncing attendance...</p></div> : <div className="rounded-xl border overflow-hidden"><Table><TableHeader className="bg-muted/50"><TableRow><TableHead className="font-black uppercase text-[10px]">User</TableHead><TableHead className="text-right font-black uppercase text-[10px]">Pulses</TableHead></TableRow></TableHeader><TableBody>{Object.values(logs.reduce((acc: any, log: any) => { if (!acc[log.userId]) acc[log.userId] = { name: log.userName, role: log.userRole, count: 0 }; acc[log.userId].count++; return acc; }, {})).map((u: any, i: number) => (<TableRow key={i}><TableCell><div className="font-bold text-xs">{u.name}</div><div className="text-[9px] uppercase text-muted-foreground font-black">{u.role}</div></TableCell><TableCell className="text-right"><Badge className="bg-primary/10 text-primary text-[10px] font-black">{u.count} pulses</Badge></TableCell></TableRow>))}</TableBody></Table></div>}</ScrollArea><DialogFooter className="p-4 border-t"><Button onClick={onClose} className="w-full h-12 font-black">Close</Button></DialogFooter></DialogContent></Dialog>);
}
...
function BulkUploadDialog({ courses, subjects, materials }: { courses: any[], subjects: any[], materials: any[] }) {
  ...
  const handleRefreshRowTitle = async (index: number) => {
    const row = bulkRows[index];
    if (!row.videoUrl || !getYouTubeID(row.videoUrl)) {
      toast({ variant: 'destructive', title: "Invalid URL", description: "Please enter a valid YouTube link first." });
      return;
    }
    
    setBulkRows(prev => {
      const next = [...prev];
      next[index] = { ...next[index], isFetchingTitle: true };
      return next;
    });

    try {
      const res = await fetchSingleVideoInfo({ url: row.videoUrl.trim() });
      setBulkRows(current => {
        const up = [...current];
        if (up[index]) {
      ...
      toast({ variant: 'destructive', title: "Fetch Failed", description: e.message || "Could not retrieve video information." });
      setBulkRows(current => {
        const up = [...current];
        if (up[index]) up[index].isFetchingTitle = false;
        return up;
      });
    }
  }

  const handleSaveBulk = () => { if (!db || !selectedCourse || !selectedSubject) return; setLoading(true); const batch = writeBatch(db); bulkRows.forEach(row => { const t = row.title.trim(); if (row.videoUrl.trim()) batch.set(row.videoId ? doc(db, 'materials', row.videoId) : doc(collection(db, 'materials')), { title: t || `Ch ${row.chapter} Video`, courseId: selectedCourse.id, subjectId: selectedSubject.id, type: 'video', url: row.videoUrl.trim(), chapter: Number(row.chapter), createdAt: serverTimestamp() }, { merge: true }); if (row.pdfUrl.trim()) batch.set(row.pdfId ? doc(db, 'materials', row.pdfId) : doc(collection(db, 'materials')), { title: t ? `${t} (Notes)` : `Ch ${row.chapter} Notes`, courseId: selectedCourse.id, subjectId: selectedSubject.id, type: 'pdf', url: row.pdfUrl.trim(), chapter: Number(row.chapter), createdAt: serverTimestamp() }, { merge: true }); }); batch.commit().then(() => { toast({ title: "Curriculum Saved" }); setOpen(false); setLoading(false); }).catch(() => setLoading(false)); }
  
  return (<Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button className="bg-primary rounded-xl text-xs h-11 font-black"><Layers size={14} className="mr-2" /> Bulk Material Tool</Button></DialogTrigger><DialogContent className="max-w-6xl h-[92vh] flex flex-col p-0 overflow-hidden w-[98vw] rounded-2xl"><DialogHeader className="p-6 pb-2"><DialogTitle className="font-black text-2xl">Bulk Material Automator</DialogTitle></DialogHeader><ScrollArea className="flex-1 px-4 md:px-6"><div className="space-y-6 pb-8"><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div className="space-y-2"><Label className="text-[10px] uppercase font-black opacity-60">Target Class & Subject</Label><div className="grid grid-cols-2 gap-2"><Select onValueChange={setSelectedClassId} value={selectedClassId}><SelectTrigger className="h-11 font-bold text-xs"><SelectValue placeholder="Class" /></SelectTrigger><SelectContent>{courses.map(c => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent></Select><Select onValueChange={setSelectedSubjectId} value={selectedSubjectId} disabled={!selectedClassId}><SelectTrigger className="h-11 font-bold text-xs"><SelectValue placeholder="Subject" /></SelectTrigger><SelectContent>{subjects.filter(s => s.courseId === selectedClassId).map(s => <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>)}</SelectContent></Select></div></div><div className="space-y-2"><Label className="text-[10px] uppercase font-black text-primary">YouTube Playlist Import</Label><div className="flex gap-2"><Input placeholder="Paste YouTube Playlist URL..." value={playlistUrl} onChange={e => setPlaylistUrl(e.target.value)} className="h-11 bg-primary/5 font-medium" disabled={!selectedSubjectId || isFetching} /><Button onClick={handleFetchPlaylist} disabled={!selectedSubjectId || isFetching || !playlistUrl.trim()} className="h-11 bg-accent font-black">{isFetching ? <Loader2 className="animate-spin h-4 w-4" /> : <Sparkles size={16} />}</Button></div></div></div>{selectedSubject && (<div className="space-y-6"><div className="flex justify-between items-center"><h4 className="text-xs font-black uppercase tracking-widest text-muted-foreground">Material Entry Grid ({bulkRows.length})</h4><Button variant="outline" size="sm" onClick={() => setBulkRows([...bulkRows, { chapter: Math.max(0, ...bulkRows.map(r => r.chapter)) + 1, title: '', videoUrl: '', pdfUrl: '' }])} className="h-9 rounded-lg font-black"><Plus size={14} /> Add Row</Button></div><div className="space-y-4">{bulkRows.map((row, idx) => (<div key={idx} className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-end p-4 bg-accent/5 rounded-xl border border-accent/10 relative group"><div className="sm:col-span-1"><Label className="text-[9px] font-black opacity-60">Ch</Label><Input type="number" className="h-10 font-black text-xs" value={row.chapter} onChange={e => handleUpdateRow(idx, 'chapter', Number(e.target.value))} /></div><div className="sm:col-span-4"><Label className="text-[9px] font-black opacity-60">Material Title</Label><div className="relative"><Input className="h-10 text-xs font-bold pr-8" placeholder="Auto-fetched title..." value={row.title} onChange={e => handleUpdateRow(idx, 'title', e.target.value)} />{row.isFetchingTitle && <Loader2 className="absolute right-2 top-3 h-4 w-4 animate-spin text-primary" />}</div></div><div className="sm:col-span-3"><Label className="text-[9px] font-black opacity-60">Video URL</Label><div className="flex gap-1 items-center"><Input className="h-10 text-[10px] font-medium" value={row.videoUrl} onChange={e => handleUpdateRow(idx, 'videoUrl', e.target.value)} /><Button variant="ghost" size="icon" className="h-10 w-10 shrink-0 hover:bg-accent/20" onClick={() => handleRefreshRowTitle(idx)} disabled={row.isFetchingTitle || !row.videoUrl}>{row.isFetchingTitle ? <Loader2 className="animate-spin h-3 w-3" /> : <RefreshCw className="h-3 w-3" />}</Button></div></div><div className="sm:col-span-3"><Label className="text-[9px] font-black opacity-60">Notes URL</Label><Input className="h-10 text-[10px] font-medium" value={row.pdfUrl} onChange={e => handleUpdateRow(idx, 'pdfUrl', e.target.value)} /></div><div className="sm:col-span-1 flex justify-center pb-2"><Button variant="ghost" size="icon" className="h-8 w-8 text-destructive opacity-0 group-hover:opacity-100" onClick={() => setBulkRows(bulkRows.filter((_, i) => i !== idx))}><Trash2 size={14} /></Button></div></div>))}</div></div>)}</div></ScrollArea><DialogFooter className="p-4 md:p-6 border-t bg-muted/20 flex gap-2"><Button variant="ghost" onClick={() => setOpen(false)} className="h-12 px-8 font-black">Cancel</Button><Button onClick={handleSaveBulk} disabled={loading || bulkRows.length === 0} className="h-12 px-12 bg-primary font-black shadow-lg shadow-primary/20">{loading ? <Loader2 className="animate-spin mr-2" /> : <Library className="mr-2" />} Save Curriculum</Button></DialogFooter></DialogContent></Dialog>);
}
